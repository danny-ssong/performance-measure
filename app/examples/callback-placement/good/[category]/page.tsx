...
export default function CallbackPlacementGoodCategoryPage({
  params,
}: {
  ...
  const { data } = useQuery<SettingsItem[]>({
    queryKey: ["callback-placement", "good", category],
    queryFn: async () => {
  ...
  const handleSave = () => {
    ...
    mutate(updates, {
      onSuccess: () => {
        ...
        router.push("/examples/callback-placement/good");
      },
    });
  };
  ...
  return (
    <ExamplePageLayout
      backHref="/examples/callback-placement/good"
      backLabel="← 환경설정 목록"
    >
      {!data ? (
        <p>불러오는 중...</p>
